...
### Key 全生命周期路径表：新增 / 恢复 / 删除（2026-09-17 补充）

上表「权威源 → 恢复路径」只给了恢复一列的概要。下表把每个 key 拆成**新增（首次写入）/ 恢复（miss 后重建）/ 删除（显式路径）**三个时机，逐一给出触发条件和代码位置，作为改动落点的直接依据。

**约定**：① "删除"只统计显式 `Del`/`SRem`/覆盖式重建，自然 TTL 过期不算删除（过期时间见上表「结算后 TTL」列）；② 锁 key 没有独立的"恢复"语义——`SetNX` 本身就是"不存在则创建"，因此锁 key 的新增列即恢复列；③ 第 01 条的注册表 key 目前**零实现**（`grep -rn "RankActiveServicesKey" --include="*.go" .` 全仓无匹配），下表把它单独放在最后并标注「设计草案，非代码事实」，不要与其余 16 个已验证的 key 混为一谈。

**分组 / 成员 / 结算数据（`common/rank` 层，rank:def / inst / mb / seq / settled）**

| key | 新增（首次写入） | 恢复（miss 后重建） | 删除（显式路径） |
|---|---|---|---|
| `rank:def:{rankCode}` | `RegisterRank`（`Set`，[service_redis.go:150-165](../../common/rank/service_redis.go)）。6 处调用点：`registerEngine`（[manager.go:160](../../socialserver/internal/rank/manager.go)）、`syncFromRedis`（[manager.go:538](../../socialserver/internal/rank/manager.go)）、`syncFromMongo`（[manager.go:859](../../socialserver/internal/rank/manager.go)）、`applyRankConfigDoc`（[manager.go:1002](../../socialserver/internal/rank/manager.go)）、`registerSubService`/`replaceSubService`（[manager_periodic.go:50](../../socialserver/internal/rank/manager_periodic.go)、`96`） | **无专门恢复函数**（缺陷 1）：`OpenInstance` 查 `Exists(rank:def)` miss 时直接返 `ErrDefinitionNotFound`（[service_redis.go:189](../../common/rank/service_redis.go)），不重试；实际靠下一次 30s `syncFromRedis`/`syncFromMongo` 循环重新走一遍新增路径，最坏延迟 30s | `DeleteRankDef`（`Del`，[service_redis.go:640-643](../../common/rank/service_redis.go)）。调用方：`Service.Cleanup()`（[service.go:919](../../socialserver/internal/rank/engine/service.go)）、`Manager.forceCleanupOrphan`（[manager.go:339](../../socialserver/internal/rank/manager.go)）——均只在 `RemoveService` 链路上（[manager.go:278](../../socialserver/internal/rank/manager.go)） |
| `rank:inst:{instanceID}` | `OpenInstance`（`SetNX`），由 `ensureGroupInstance` 在 `GetInstance` 返回 `ErrInstanceNotFound` 时调用（[group.go:76-99](../../socialserver/internal/rank/engine/group.go)） | 冷启动：`ensureLoaded` 对已结算/未结算实例分别调用 `RestoreInstance`（[service.go:119](../../socialserver/internal/rank/engine/service.go)、`193`、`229`）；运行时：`UpsertScore` 检测 `rank:mb` 不存在（[service.go:436-439](../../socialserver/internal/rank/engine/service.go)）触发 `recoverGroupData`（[service.go:245-315](../../socialserver/internal/rank/engine/service.go)），内部同样调 `RestoreInstance`（[service.go:310](../../socialserver/internal/rank/engine/service.go)） | `DeleteInstance`（[service_redis.go:622-628](../../common/rank/service_redis.go)，一次调用同时删 inst/mb/seq/settled），调用方同 `rank:def`（GM 路径专用）；结算后走 `ExpireInstance` 设 TTL 而非删除，仅周期路径（[service.go:941](../../socialserver/internal/rank/engine/service.go)） |
| `rank:mb:{instanceID}` | `BatchUpsertScore`（Lua，[service_redis.go:344-](../../common/rank/service_redis.go)），由 `UpsertScore` 每次调用（[service.go:445-453](../../socialserver/internal/rank/engine/service.go)） | `RestoreMembers`（[service_redis.go:649-](../../common/rank/service_redis.go)）强写全部成员分数，调用方同 `rank:inst`（`service.go:173`、`288`，均在 `ensureLoaded`/`recoverGroupData` 内） | 同 `rank:inst`（`DeleteInstance` 一次调用覆盖） |
| `rank:seq:{instanceID}` | 同 `rank:mb`（`BatchUpsertScore` 内维护自增序号） | `RestoreMembers` 恢复成员分数后，把序号计数器推进到 `max(sequence)`（同一函数内完成，[service_redis.go:649-](../../common/rank/service_redis.go)） | 同 `rank:inst`（`DeleteInstance` 一次调用覆盖） |
| `rank:settled:{instanceID}` | `SettleInstance`（`Set`，[service_redis.go:286-343](../../common/rank/service_redis.go)），由 `Settle()` 结算分组时调用（[service.go:665](../../socialserver/internal/rank/engine/service.go)） | 两条独立路径：① 冷启动 `RestoreSettled` 强写（[store.go:206-218](../../socialserver/internal/rank/engine/store.go)），调用方 `service.go:620`、`625`；② 历史查询 miss 时 `LoadGroupSettledCached` 用 `SetEX(SettledCacheTTL)` 回填（[store.go:757](../../socialserver/internal/rank/engine/store.go)，全表**唯一**带 TTL 的写入路径，但会被①的强写覆盖掉 TTL，见缺陷 4） | 同 `rank:inst`（`DeleteInstance` 一次调用覆盖） |

**业务层数据（`internal/rank/engine` 层，rank:meta / groups / members / claims / robots / robot_infos / mongo_chk）**

| key | 新增（首次写入） | 恢复（miss 后重建） | 删除（显式路径） |
|---|---|---|---|
| `rank:meta:{bizId}` | `SaveActivityTimes`（`HSet` openTime/closeTime/gameEndTime，[store.go:164-172](../../socialserver/internal/rank/engine/store.go)），`NewService` 构造时调用一次（所有 6 处注册路径都经过它）；随后 `NextGroupID`（`HIncrBy nextGroupID`，[store.go:152-161](../../socialserver/internal/rank/engine/store.go)，`ensureGroupLocked` 每建一个新分组调一次，[group.go:34](../../socialserver/internal/rank/engine/group.go)）与 `IncrRealCount`（`HIncrBy realCount_{gid}`，[store.go:127-150](../../socialserver/internal/rank/engine/store.go)）持续追加字段 | `RestoreNextGroupID`（[store.go:194-204](../../socialserver/internal/rank/engine/store.go)）在 `ensureLoaded` 末尾把 `nextGroupID` 修正到 `≥ max(现有分组ID)`（[service.go:70](../../socialserver/internal/rank/engine/service.go)），防止 Redis 清空后新分组 ID 与旧分组冲突；`openTime`/`closeTime`/`gameEndTime` 无独立恢复函数，靠 `SaveActivityTimes` 幂等重写覆盖 | `CleanupAll`（`Del`，[store.go:435](../../socialserver/internal/rank/engine/store.go)），调用方是 `Service.Cleanup()`（[service.go:907](../../socialserver/internal/rank/engine/service.go)）与 `Manager.forceCleanupOrphan`（[manager.go:328](../../socialserver/internal/rank/manager.go)），均只在 `RemoveService` 链路上 |
| `rank:groups:{bizId}` | `SaveGroup`（`HSet`，[store.go:54-65](../../socialserver/internal/rank/engine/store.go)），由 `ensureGroupLocked` 新建分组时调用（[group.go:50](../../socialserver/internal/rank/engine/group.go)） | `LoadGroups` miss 时从 `CT_RANK_GROUP` 批量回填（[store.go:90-103](../../socialserver/internal/rank/engine/store.go)），几乎每次 `ensureGroupLocked`（[group.go:20](../../socialserver/internal/rank/engine/group.go)）与冷启动 `ensureLoaded` 都会先调用一次 `LoadGroups` | `CleanupAll`（`Del`，[store.go:436](../../socialserver/internal/rank/engine/store.go)） |
| `rank:members:{bizId}` | `SetMember`（`HSet`，[store.go:222-232](../../socialserver/internal/rank/engine/store.go)），由 `UpsertScore` 新成员入组时调用（[service.go:413](../../socialserver/internal/rank/engine/service.go)） | `GetMember`/`GetAllMembers` miss 时分别从 `CT_RANK_MEMBER` 回填单条/全量（[store.go:252-260](../../socialserver/internal/rank/engine/store.go)、`293-304`）；查不到时写负向缓存哨兵 `nullCacheEntry`，本身也算一种"恢复"（记住"查过没有"） | `CleanupAll`（`Del`，[store.go:437](../../socialserver/internal/rank/engine/store.go)） |
| `rank:claims:{bizId}` | `AtomicClaim`（Lua，首次领奖原子写入，[store.go:540-605](../../socialserver/internal/rank/engine/store.go)）；`SetClaim`（[store.go:639-651](../../socialserver/internal/rank/engine/store.go)）供补偿/GM 路径显式写入 | `AtomicClaim` 内建"Redis 未命中但 Mongo 有记录"分支：Lua 判定"首次领取"后反查 `CT_RANK_CLAIM`，命中则 `HSet` 回写（[store.go:588-590](../../socialserver/internal/rank/engine/store.go)），真首次则 `SaveClaimIfNotExists` 落库防重复（[store.go:594](../../socialserver/internal/rank/engine/store.go)） | `CleanupAll`（`Del`，[store.go:438](../../socialserver/internal/rank/engine/store.go)） |
| `rank:robots:{bizId}:{gid}` | `SaveRobots`（`HSet`，[store.go:309-323](../../socialserver/internal/rank/engine/store.go)），由 `spawnRobotsForGroup` 在首位真实玩家入组时调用（[service_robot.go:97](../../socialserver/internal/rank/engine/service_robot.go)），`tickAllRobots` 增量更新（[service_robot.go:190](../../socialserver/internal/rank/engine/service_robot.go)） | `LoadRobots` miss 时从 `CT_RANK_ROBOT` 批量回填（[store.go:342-352](../../socialserver/internal/rank/engine/store.go)） | `CleanupAll`（逐分组 `Del`，[store.go:444](../../socialserver/internal/rank/engine/store.go)） |
| `rank:robot_infos:{bizId}:{gid}` | `SaveUsedInfoIDs`（`SAdd`，[store.go:356-366](../../socialserver/internal/rank/engine/store.go)），调用方同 `rank:robots`（[service_robot.go:98](../../socialserver/internal/rank/engine/service_robot.go)） | **无 Mongo 权威源**——`LoadUsedInfoIDs`（[store.go:368-385](../../socialserver/internal/rank/engine/store.go)）miss 时直接返回空集合，靠后续机器人生成重新累积；属于"可重建"而非"精确还原" | `CleanupAll`（逐分组 `Del`，[store.go:445](../../socialserver/internal/rank/engine/store.go)） |
| `rank:mongo_chk:{bizId}` | `setMongoChecked`（`SetEX(10min)`，[store.go:41-43](../../socialserver/internal/rank/engine/store.go)），仅在 `LoadGroups`/`GetAllMembers` 查询 Mongo 确认"当前确实无数据"时才写（[store.go:96](../../socialserver/internal/rank/engine/store.go)、`298`） | 不需要——它本身就是缓存哨兵；10 分钟内直接跳过 Mongo 查询，到期后下次读操作重新判定 | 无独立删除路径；`CleanupAll`/`CleanupLiveData` 清理该 bizId 时顺带 `Del`/`Expire`（[store.go:439](../../socialserver/internal/rank/engine/store.go)、`407`），否则只能自然过期 |
| `rank:max_score:{bizId}`（死代码） | 无——全仓无生产者 | 无——全仓无消费者 | 无（[defines_rank.go:88-123](../../common/redis/defines_rank.go) 仅有定义，见缺陷 5，建议删除定义或明确标注预留） |

**编排层（`internal/rank` 层，rank:member_index + 3 个锁 key）**

| key | 新增 / 恢复 | 删除 |
|---|---|---|
| `rank:member_index:{uid}` | `Track`（`SAdd`，[member_index.go:51-66](../../socialserver/internal/rank/member_index.go)），由 `onMemberJoin` 回调在成员首次入组时触发（`UpsertScore` → 闭包 → `Track`）。**恢复**走独立路径：`rebuildMemberIndex`（[manager.go:396-421](../../socialserver/internal/rank/manager.go)）遍历该用户参与的所有 `engine.Service` 内存状态重建，权威源是 engine 内存而非 MongoDB（全表唯一例外） | `RemoveByKey`（GM 删除单个活动时移除该活动的索引条目，[member_index.go:102-119](../../socialserver/internal/rank/member_index.go)）与 `RemoveUserEntries`（活动整体清理时批量移除，[member_index.go:123-131](../../socialserver/internal/rank/member_index.go)），调用方均为 `RemoveService`（[manager.go:296-299](../../socialserver/internal/rank/manager.go)）/ `forceCleanupOrphan`（[manager.go:323-325](../../socialserver/internal/rank/manager.go)）/ `subscribeDeleteEvents`（[manager.go:741-744](../../socialserver/internal/rank/manager.go)）/ `syncFromMongo` 的孤儿移除分支（[manager.go:926-928](../../socialserver/internal/rank/manager.go)） |
| `rank:settle:{bizId}:{gid}` | `TryLockSettle`（`SetNX`，[store.go:610-624](../../socialserver/internal/rank/engine/store.go)），`Settle()` 每个未结算分组调用一次（[service.go:659](../../socialserver/internal/rank/engine/service.go)）——`SetNX` 本身即新增即恢复，无需单独恢复函数 | 无显式删除，10 分钟 TTL 自然过期（锁语义的正常设计，不是缺口） |
| `rank:robot_tick:{bizId}:{sec}` | `TryLockRobotTick`（`SetNX`，[store.go:626-637](../../socialserver/internal/rank/engine/store.go)），`Tick()` 每秒调用一次（[service.go:352](../../socialserver/internal/rank/engine/service.go)） | 无显式删除，3 秒 TTL 自然过期 |
| `rank:periodic_advance:{lk}:r{n}` | 内联 `SetNX`（[periodic/handler.go:185](../../socialserver/internal/rank/periodic/handler.go)），`advanceRound` 每次尝试推进轮次调用 | 无显式删除，1 分钟 TTL 自然过期 |

**⚠️ 设计草案（非代码事实）**

| key | 状态 |
|---|---|
| `rank:{active_services}`（第 01 条提议） | **零实现**：`grep -rn "RankActiveServicesKey" --include="*.go" .` 全仓无匹配。方案文字（非代码）：新增 = `syncFromMongo`/`NewService` 路径顺带 `SAdd`；删除 = 成员级懒惰修剪（读到远期哨兵值时踢除）；key 本身目前设计成永久，缺陷 8 要求改滑动 TTL。**不要把这一行当作已验证的代码路径引用**——它与上面 16 个 key 的性质不同，那些都有可运行的调用点，这个只是本文档的设计建议。
